...
export default function CustomerAdd() {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<Customer>({ resolver: yupResolver(CustomerSchema) });
  const navigate = useNavigate();

  const onSubmit: SubmitHandler<Customer> = async (data) => {
    const customer = serialized(data, "customers");
    try {
      await postCustomerRequest(customer);
  ...
  return (
    <SectionCard title="Agregar cliente">
      <FormCard>
